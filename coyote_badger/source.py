...
class Source(object):
    ...
    def infer_kind(self):
        ...
        if (re.search('[0-9]+ [sS] ?[cC][tT] [0-9]+', long_cite_no_periods)
        ...
        if (' v. ' in long_cite_lower
                or 'in re ' in long_cite_lower):
        ...
        # a federal statute.
        if (' u.s.c. ' in long_cite_lower
                or ' u.s. code ' in long_cite_lower):
            return Kind.FEDERAL
        ...
        if (' stat. ' in long_cite_lower
                or '§' in self.long_cite):
            return Kind.STATE
